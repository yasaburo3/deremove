...
void CloudPublisher::PublishData(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud_ptr_input, ros::Time time) {
...
// PointXYZRGB重载
void CloudPublisher::Publish(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_ptr_input, double time) {
    ros::Time ros_time((float)time);
    PublishData(cloud_ptr_input, ros_time);
}

void CloudPublisher::Publish(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_ptr_input) {
    ros::Time time = ros::Time::now();
    PublishData(cloud_ptr_input, time);
}

void CloudPublisher::PublishData(pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloud_ptr_input, ros::Time time) {
    sensor_msgs::PointCloud2Ptr cloud_ptr_output(new sensor_msgs::PointCloud2());
    pcl::toROSMsg(*cloud_ptr_input, *cloud_ptr_output);
